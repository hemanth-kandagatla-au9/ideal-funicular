...
import { fetchDataFailure, fetchDataStart, fetchDataSuccess } from '../../store/JobSlice/jobsSlice';
...
export const AxiosInstance = new AxiosInstanceClass(
  `${
    process.env.REACT_APP_BACKEND_URL ? process.env.REACT_APP_BACKEND_URL : 'http://localhost:3001'
  }`
).init(token);
...
export const addHosts = createAsyncThunk('addhost', async (hostData, thunkAPI) => {
  try {
...
export const getHostById = createAsyncThunk('getHostById', async (hostId, thunkAPI) => {
  try {
    if (hostId) {
      const response = await AxiosInstance.get(`${get.getHostById}?hostId=${hostId}`);
...
export const addCategory = createAsyncThunk('AddCategory', async (categoryData, thunkAPI) => {
  try {
    const response = await AxiosInstance.post(`${post.createCategory}`, categoryData);
    return response;
  } catch (err) {
    console.log(err);
    return thunkAPI.rejectWithValue({ error: err.message });
...
export const getCategoryById = createAsyncThunk('getCategory', async (categoryId, thunkAPI) => {
  try {
    if (categoryId) {
      const response = await AxiosInstance.get(`${get.getCategoryById}?categoryId=${categoryId}`);
      return response;
    }
  } catch (err) {
    console.log(err);
...
export const addOpenSearchIndex = createAsyncThunk(
  ...
  async (indexData, thunkAPI) => {
    try {
      const response = await AxiosInstance.post(`${post.createOpenSearchIndex}`, indexData);
      return response;
    } catch (err) {
      console.log(err);
      return thunkAPI.rejectWithValue({ error: err.message });
...
export const deleteOpenSearchIndex = async (indexId) => {
  try {
    if (indexId) {
      const response = await AxiosInstance.post(`${del.deleteOpenSearchIndex}`, {
        indexId: indexId,
      });
      return response;
    }
...
export const addCommandCategory = async (commandCategoryData) => {
  try {
    const response = await AxiosInstance.post(post.createCommandCategory, commandCategoryData);
    return response;
  } catch (err) {
    console.error('Error creating command category:', err?.response?.data || err.message);
    throw err;
  }
};
...
export const deleteCommandCategory = async (commandCategoryId) => {
  try {
    if (commandCategoryId) {
      const response = await AxiosInstance.post(`${del.deleteCommandCategory}`, {
...
export const getOpenSearchIndex = async (queryParams) => {
  try {
    ...
    const response = await AxiosInstance.get(`${get.getOpenSearchIndex}?${queryString}`);
...
export const getOpenSearchIndexByRunId = async (queryParams) => {
  try {
    ...
    const queryString = new URLSearchParams(queryParams).toString();
    const response = await AxiosInstance.get(`${get.getOpenSearchIndexByRunId}?${queryString}`);
    return response || [];
  } catch (err) {
    console.error('Error fetching logs:', err);
    return { error: err.message };
...
export const getReportTags = (searchText) => async (dispatch) => {
  ...
  try {
    ...
    let url = searchText ? `${getReportTags}?tagName=${searchText}` : getReportTags;
    const response = await axiosInstance.get(url);
...
export const saveOrUpdateReport = createAsyncThunk(
  ...
  async (reportData, thunkAPI) => {
    try {
      if (reportData) {
        const response = await AxiosInstance.post(`/report/handleReport`, reportData);
        return response.data;
      }
    } catch (err) {
      console.log(err);
...
export const getReportDefaultColumnOptions = createAsyncThunk(
  ...
  async (reqObj, thunkAPI) => {
    try {
      const response = await AxiosInstance.post(`/report/getReportDefaultColumnOptions`, reqObj);
      return response.data;
    } catch (err) {
...
export const getReportDetailsById = async ({ reportId, pageNo = 1, pageSize = 3 }) => {
  try {
    if (reportId) {
      const response = await AxiosInstance.get(
...
export const getUsersList = async ({ pageNo = 1, pageSize = 10 }) => {
  try {
    const response = await AxiosInstance.get(
      `${get.getUsersList}?pageNo=${pageNo}&pageSize=${pageSize}`
    );
...
export const bulkAgentAction = async (data) => {
  try {
    return await AxiosInstance.post(`${post.bulkAgentAction}?type=${data.type}`, data);
  } catch (error) {
    return error.response;
  }
};
...
export const getFilteredSchedules = createAsyncThunk(
  ...
  async (filters = {}, thunkAPI) => {
    try {
      const response = await AxiosInstance.post(`/report/filter-schedules`, filters);
      return response.data;
    } catch (err) {
