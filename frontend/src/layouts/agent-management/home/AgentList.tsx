...
const AgentList = ({
  ...
  const hasBulkPermission = [
    AGENT_PERMISSIONS.RISE_AGENT_BULK_START,
    AGENT_PERMISSIONS.RISE_AGENT_BULK_STOP,
    AGENT_PERMISSIONS.RISE_AGENT_BULK_RESTART,
    AGENT_PERMISSIONS.RISE_AGENT_BULK_UPGRADE,
    AGENT_PERMISSIONS.RISE_AGENT_BULK_SYNC_CONFIG,
    AGENT_PERMISSIONS.RISE_AGENT_BULK_ENV_UPGRADE,
  ].some(permission => hasPermission(permission));

  const hasActionColumn =
    hasPermission(AGENT_PERMISSIONS.RISE_AGENT_ACTION_VIEW) || hasPermission(AGENT_PERMISSIONS.RISE_AGENT_CHECK_STATUS) || hasPermission(AGENT_PERMISSIONS.RISE_AGENT_ACCORDION);

  const agentCardsLabel = ["Hostname", "OS", "Uptime", "Version", "Status", ...(hasActionColumn ? ["Action"] : [])];

  const gridTemplateColumns = [
    hasBulkPermission ? "40px" : null,
    "minmax(0, 2fr)",
    "minmax(0, 1fr)",
    "minmax(0, 1fr)",
    "minmax(0, 1fr)",
    "minmax(0, 1fr)",
    hasActionColumn ? "minmax(0, 1fr)" : null,
  ]
    .filter(Boolean)
    .join(" ");
  ...
  if (loading) {
    return (
      <>
        <div className="riseagent-agentHeaderRow" style={rowStyle}>
          {hasBulkPermission && (
...
<span className="riseagent-fieldLabel">{ele}</span>
        ...
        {Array.from({ length: pagination.limit }).map((_, i) => (
          <div className="riseagent-agentRowWrapper" key={i}>
            <div className="riseagent-cardsAgent" style={rowStyle}>
              {hasBulkPermission && (
                <div className="riseagent-agentRowFirst">
                    ...
                    {hasActionColumn && (
                      <div className="riseagent-agentActions riseagent-agentRow">
                        {isCheckStatusAgentEnabled && (
                          ...
                          >
                            <img src={checkSquare} alt="view" style={{ maxWidth: "none", display: "inline-block" }} />
                          </Button>
                        )}
                        {!isEmpty(agents) && isViewAgentEnabled && (
                          ...
                          >
                            <img src={vieweye} alt="view" style={{ maxWidth: "none", display: "inline-block" }} />
                          </Button>
                        )}
                        {hasPermission(AGENT_PERMISSIONS.RISE_AGENT_ACCORDION) && (
                          ...
                          >
                            <img
                              ...
                              style={{ maxWidth: "none", display: "inline-block" }}
                            />
                          </Button>
                        )}
                      </div>
