...
describe("User Authorization Sagas - REAL FUNCTIONAL TESTS", () => {
  ...
  describe("createUserSaga", () => {
    it("creates user successfully", () => {
      ...
      expect(step3.value).toEqual(put(userAuthorizationActions.fetchUsers({ page: 1, limit: 10, search: "" })));
  ...
  describe("createPermissionSaga", () => {
    it("creates permission successfully", () => {
      ...
      const step3 = generator.next();
      expect(step3.value).toEqual(put(userAuthorizationActions.fetchPermissions({ page: 1, limit: 10 })));
    ...
    it("handles createPermissionSaga API error response", () => {
      ...
      const step2 = generator.next(errorResponse);
      expect(step2.value).toEqual(put(userAuthorizationActions.failureCreatePermission(errorResponse.data.error)));
  ...
  describe("deletePermissionSaga", () => {
    ...
    it("handles deletePermissionSaga with unknown error", () => {
      ...
      const step2 = generator.next(errorResponse);
      expect(step2.value).toEqual(put(userAuthorizationActions.failureDeletePermission(null)));
  ...
  describe("assignUserPermissionsSaga", () => {
    it("assigns permissions to user successfully", () => {
      ...
      (userAuthorizationService.assignUserPermissions as jest.Mock).mockResolvedValue(mockResponse);
      ...
      const step1 = generator.next();
      expect(step1.value).toEqual(call(userAuthorizationService.assignUserPermissions, userId, permissionCodes));

      const step2 = generator.next(mockResponse);
      expect(step2.value).toEqual(put(userAuthorizationActions.successAssignUserPermissions(mockResponse)));
    ...
    it("handles assignUserPermissionsSaga API error", () => {
      ...
      (userAuthorizationService.assignUserPermissions as jest.Mock).mockResolvedValue(errorResponse);
  ...
  describe("fetchGlobalPermissionsSaga", () => {
    it("fetches global permissions for user successfully", () => {
      ...
      (userAuthorizationService.fetchGlobalPermissions as jest.Mock).mockResolvedValue(mockResponse);
      ...
      const step2 = generator.next(mockResponse);
      expect(step2.value).toEqual(put(userAuthorizationActions.successFetchGlobalPermissions(mockResponse)));
    ...
    it("handles fetchGlobalPermissionsSaga API error flag", () => {
      ...
      (userAuthorizationService.fetchGlobalPermissions as jest.Mock).mockResolvedValue(errorResponse);
      ...
      const step2 = generator.next(errorResponse);
      expect(step2.value).toEqual(put(userAuthorizationActions.successFetchGlobalPermissions(errorResponse)));
  ...
  describe("userAuthorizationSagaWatcher", () => {
    it("watches all action types and triggers correct sagas", () => {
      ...
      step = generator.next();
      expect(step.value).toEqual(takeLatest(AUTH.USER.ASSIGN_USER_PERMISSIONS_REQUEST, sagas.assignUserPermissionsSaga));
      step = generator.next();
      expect(step.value).toEqual(takeLatest(AUTH.USER.FETCH_GLOBAL_PERMISSIONS_REQUEST, sagas.fetchGlobalPermissionsSaga));
      step = generator.next();
      expect(step.value).toEqual(takeLatest(AUTH.PERMISSION.GET_PERMISSIONS_REQUEST, sagas.fetchPermissionsSaga));
      step = generator.next();
      expect(step.value).toEqual(takeLatest(AUTH.PERMISSION.CREATE_PERMISSION_REQUEST, sagas.createPermissionSaga));
      step = generator.next();
      expect(step.value).toEqual(takeLatest(AUTH.PERMISSION.DELETE_PERMISSION_REQUEST, sagas.deletePermissionSaga));
  ...
  describe("Saga Integration Tests", () => {
    it("create user saga calls all expected sagas in sequence", () => {
      ...
      step = generator.next();
      expect(step.value).toEqual(put(userAuthorizationActions.fetchUsers({ page: 1, limit: 10, search: "" })));
      step = generator.next();
