import AuthUtils, { LoginWithRedirectURL } from "../../utils/AuthUtils";
import { cookies, history } from "../../utils/utils";
...
describe("Auth Utils Test", () => {
  ...
  describe("Login", () => {
    it("should open login URL", () => {
      AuthUtils.Login();
      expect(window.open).toHaveBeenCalledWith(expect.stringContaining("/v1/auth/authorize"), "_self");
    });
  });
  ...
  describe("Logout", () => {
    it("should call utilizationMetrics when user has email", () => {
      ...
      expect(UtilizationService.utilizationMetrics).toHaveBeenCalledWith("Logout", {
        Action: "Logout",
        userID: "test",
        Origin: "Auth Logout",
      });
  ...
  describe("removeUserSession", () => {
    it("should clear all user session data", () => {
      ...
      expect(cookies.set).toHaveBeenCalledWith("token", "bm8tYWNjZXNzLXRva2Vu", { path: "/", domain: "localhost" });
      expect(cookies.remove).toHaveBeenCalledWith("refreshToken", { path: "/", domain: "localhost" });
      expect(cookies.remove).toHaveBeenCalledWith("user", { path: "/", domain: "localhost" });
  ...
  describe("Logout timeout redirect", () => {
    it("should redirect to logout after timeout", () => {
      ...
      expect(history.push).toHaveBeenCalledWith("/logout", { path: "/", domain: window.location.hostname });
      jest.useRealTimers();
    });
  });
  describe("LoginWithRedirectURL", () => {
    ...
    it("should set redirectUrl in sessionStorage and open login page", () => {
      ...
      expect(sessionStorage.setItem).toHaveBeenCalledWith("redirectUrl", redirectUrl);
      expect(window.open).toHaveBeenCalledWith(`/login?redirectUrl=${redirectUrl}`, "_self");
    });
  });
});
